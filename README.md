...
| RHI 抽象层 | ✅ | 后端无关接口: Buffer / Shader / Texture / Framebuffer / PipelineState / Device — OpenGL 与 Vulkan 双实现 |
...
| 正交相机 | ✅ | OrthographicCamera 2D 投影 |

### 星露谷游戏框架 (game/ 库)

> 独立于引擎的游戏逻辑层，不影响引擎的通用性。

| 特性 | 状态 | 说明 |
| --- | :---: | --- |
| 玩家控制 | ✅ | 4 方向移动 + 工具使用 + 体力系统 |
| 交互系统 | ✅ | 面前格子检测 + E 键交互 + 场景传送门 |
| 农场系统 | ✅ | 土壤状态机 (翻耕→浇水→种植→生长→收获) + 季节限制 |
| 时间系统 | ✅ | 日夜循环 + 四季轮转 + 随机天气 + Sleep 推进 |
| 物品系统 | ✅ | 全局物品数据库 + 背包堆叠/快捷栏/金币 |
| NPC 系统 | ✅ | 日程路径移动 + 好感度 + 送礼偏好 (喜爱/喜欢/讨厌/厌恶) |
| 对话系统 | ✅ | 对话树 + 逐字显示 (UTF-8 安全) + 多选项分支 |

### 丧尸生存原型 (sandbox/)

> 基于引擎 2D 工具构建的完整生存游戏原型。
...
```text
engine/       ← C++ 引擎静态库 (核心渲染/物理/ECS/音频/编辑器/2D工具)
game/         ← 游戏框架库 (星露谷专属: 农场/NPC/对话/物品, 与引擎解耦)
...
> **引擎通用性**: `engine/` 不包含任何游戏专属逻辑。做 3D 游戏时 Sandbox 直接链接 Engine，无需 Game 库。
